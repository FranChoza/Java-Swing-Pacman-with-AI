...
public class PacmanPanel extends JPanel{ 
    ...
    public PacmanPanel(){
        ...
        fantasma = new Fantasma(pacman,50,50);
        
        //Hilos
        Thread t1 = new Thread(pacman);
        t1.start();
        Thread t2 = new Thread(fantasma);
        t2.start();
    }
        ...
        for(int i=0; i<L.FILAS; ++i){
            for(int j=0; j<L.COLUMNAS; ++j){
                if(L.lab[i][j]==L.PARED){
                    g.drawImage(new ImageIcon("C:/Users/RGAP/Desktop/PACMAN/recursos/wall.jpg").getImage(), x, y, 50, 50, this);
                    x+=50;
                }
                else
                    x+=50;
